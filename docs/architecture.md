...
- **逻辑**：
  ...
  2. 拼装 Prompt 并调用 LLM。
  3. 支持工具调用模式：
     - `lookup_business_logic` 用于检索业务规则。
     - 执行类工具（`execute_pandas_query` 等）会被序列化为 JSON 计划，交给执行节点运行。
...
- **逻辑**：
  1. 阻止危险关键字（写操作、系统调用等）。
  2. Excel 模式直接视为通过。
  3. SQL 模式通过 LLM 验证 schema 与语法一致性。
...
2. 统一的 `render_prompt_template()` 进行变量注入。
